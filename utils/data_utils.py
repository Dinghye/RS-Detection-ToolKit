...
wordname = ['__background__','A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K']
...
def parse_dota_poly2(info):
    """
        parse the dota ground truth in the format:
        [x1, y1, x2, y2, x3, y3, x4, y4]
    """
    # objects = parse_dota_poly(filename)
    objects = stupid_structure(info)
    for obj in objects:
...
def stupid_structure(info):
    objects = []
    for single in info["shapes"]:
        object_structure = {}
        object_structure['name'] = single["label"]
        object_structure['difficult'] = '0'  # useless fake
        object_structure['poly'] = single["points"]
        gtpoly = shgeo.Polygon(object_structure['poly'])
        object_structure['area'] = gtpoly.area
        objects.append(object_structure)
    return objects
