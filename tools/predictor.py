...
from tools.data_loader import myVisualization
...
if __name__ == "__main__":
    ...
    cfg = get_cfg()
    cfg.merge_from_file("../COCO-Detection/my_config.yaml")
    # cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5  # 模型阈值
    cfg.MODEL.WEIGHTS = "output/model_final.pth"
    # cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5

    predictor = DefaultPredictor(cfg)
    MetadataCatalog.get("ship_train").set(things_class=CLASS_LIST)
    ...
    for d in random.sample(range(1, 261), 20):
        ...
        v = myVisualization(im[:, :, ::-1], MetadataCatalog.get("ship_train"), scale=0.8)
